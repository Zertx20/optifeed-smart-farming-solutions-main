...
const OrgStructure = () => {
  ...
  const handleDirectorateClick = (key: string) => {
    setSelectedDirectorate(key);
    setIsModalOpen(true);
  };
  ...
  return (
    <>
          ...
          >
            {directorates.map((d, i) => (
              <motion.div
                ...
                whileTap={{ scale: 0.98 }}
                onClick={() => handleDirectorateClick(d.key)}
                className={`group relative rounded-2xl overflow-hidden border border-border hover:shadow-glow transition-all duration-300 cursor-pointer ${
      ...
      <DirectorateModal
        ...
        directorateKey={selectedDirectorate}
      />
    </>
  );
};
